...
public sealed class CoreTests
{
    ...
    public async Task ClientManifest_EmbedsRuntimeAndRespectsRequiredToggle()
    {
        ...
        var runtimePackage = Path.Combine(runtime, "ValheimServerManager-2.1.4-client.zip");
        using (var archive = ZipFile.Open(runtimePackage, ZipArchiveMode.Create))
        {
            var manifest = archive.CreateEntry("manifest.json");
            await using var output = manifest.Open();
            await JsonSerializer.SerializeAsync(output, new { name = "Server_Manager", version_number = "2.1.4" });
        }
        ...
        Assert.Equal("Creaton-Server_Manager-2.1.4", serverCharacterPackages[0].GetProperty("coordinate").GetString());
