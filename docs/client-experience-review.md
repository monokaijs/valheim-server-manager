...
| Important notices | A center HUD message could disappear before the player reached the menu. | The bundled client keeps access and moderation messages available at the menu until dismissed, with a copy-message action. During play, kick and ban reasons use Valheim's message HUD. |
| Routine notices | Welcomes competed with urgent notices and could be delivered while loading. | Welcome and broadcast messages use Valheim's message HUD. The custom Server Manager panel no longer renders during play. |
| Layout | Long notices need readable layouts. | Menu notices use wrapping and scroll areas without a full-screen dim layer. Menu-only input guards prevent clicks behind a modal. |
