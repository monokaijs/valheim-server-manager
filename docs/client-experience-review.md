...
| Layout | Long notices and mod lists need readable layouts. | Scaled panels use wrapping and scroll areas. Menu-only input guards prevent clicks behind a modal. |
| Mod compatibility | Players need to know which package versions the server requires. | The read-only F8 view lists required and optional versions and checks package metadata in the active BepInEx profile. It has no install controls or download path. |
...
3. Connect with the bundled client. Confirm the F8 view lists required and optional package versions, reports missing versions, and contains no install controls. Verify whitelist notices render once.
...
6. Test exact, missing, and different package versions. Confirm only exact required versions acknowledge the server requirement and the F8 view never changes files.
7. Reconnect to a different server and verify the F8 view reflects only the current server catalog.
